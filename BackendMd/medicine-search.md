...
- 接口用途:
  提供药品关键词搜索能力，支持产品名称、批准文号、生产单位、上市许可持有人、药品编码等字段查询。
- Flutter 端调用入口:
  `lib/api/medicine_api.dart` 的 `MedicineApi.search()`，页面使用位置主要在 `lib/pages/Search/search.dart` 和药品选择相关页面。
...
- Sealos 云函数部署要点:
  在 `backend/` 执行 `npm install`、`npm run build:cloud` 后，上传 `backend/dist/cloud-bundle/medicine-search.js`；Sealos 里要同步配置 MySQL 环境变量。
- 后续迁移到云服务器时是否还能复用:
  可以；搜索逻辑已经抽成可复用 handler，后面改为 Express 路由时不用再重写接口协议。
