# Backend NestJS + PostgreSQL Migration Plan

Last updated: 2026-05-24

## 1. 迁移目标

后端从当前 Express + TypeScript 服务迁移到 NestJS，并把 PostgreSQL 作为唯一主业务数据库。

目标状态：

- 框架：NestJS。
- 主数据库：PostgreSQL。
- 缓存与短期状态：Redis 仅保留验证码、发送冷却、AI 文本缓存等短生命周期能力。
...
| AI 文本缓存 | Redis | Redis，后续如需审计再落 PostgreSQL |
...
| `AuthModule` | `handlers/auth.ts` | 验证码、注册、登录、刷新 token |
| `UsersModule` | `handlers/auth.ts` profile/delete 部分 | 用户资料查询、更新、注销 |
| `MedicinesModule` | `medicine-search/detail/scan` | 药品搜索、详情、扫码候选 |
| `AiModule` | `ai/*` + AI handlers | AI 详情、AI 安全分析、LangChain gateway、AI cache |
...
| `POST /api/medicines/ai-detail` | `AiController.detail` |
| `POST /api/medicines/ai-safety` | `AiController.safety` |
...
药品库搜索第一阶段先复刻当前 MySQL `LIKE` 行为。后续如果要优化中文搜索，再评估 `pg_trgm`、外部分词索引或专门搜索服务，不要在框架迁移首批次扩大范围。
...
- 将 `medicine-ai-detail`、`medicine-ai-safety` 的核心逻辑拆成 service，暂不切外部路由。
...
目标：把 MySQL 药品库迁到 PostgreSQL，并先迁移公开、低用户状态的接口。

任务：

- 编写 MySQL 到 PostgreSQL 的药品目录导入脚本。
...
1. `medicine_catalog`，来源 MySQL。
2. `users`，来源 MongoDB `User`。
3. `my_medicines`，来源 MongoDB `MyMedicine`。
4. `reminders` 和 `reminder_medicines`，来源 MongoDB `Reminder`。
5. `scan_records`，来源 MongoDB `ScanRecord`。
...
- Flutter 核心链路手动验证：注册/登录、药品搜索、药品详情、AI 解读、扫码、我的药品、提醒、扫描记录。
...
- [ ] Phase 3：迁移 AI/common/config/Redis 公共能力。
- [ ] Phase 4：迁移药品目录和公开药品接口。
