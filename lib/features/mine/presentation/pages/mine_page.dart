...
class MinePage extends ConsumerWidget {
  ...
  List<MineQuickActionData> _quickActions(AppLocalizations? l10n) {
    return [
      MineQuickActionData(icon: Icons.alarm_rounded, title: l10n?.mineQuickReminderTitle ?? '今日提醒', subtitle: l10n?.mineQuickReminderSubtitle ?? '查看计划', color: const Color(0xFF10B981), id: 'reminders'),
      MineQuickActionData(icon: Icons.search_rounded, title: l10n?.mineQuickSearchTitle ?? '手动搜索', subtitle: l10n?.mineQuickSearchSubtitle ?? '药品信息', color: const Color(0xFF0EA5E9), id: 'search'),
      MineQuickActionData(icon: Icons.settings_rounded, title: l10n?.mineQuickSettingsTitle ?? '设置', subtitle: l10n?.mineQuickSettingsSubtitle ?? '偏好选项', color: const Color(0xFF6366F1), id: 'settings'),
    ];
  }
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final badgeText = preview.count > 0
        ? (l10n?.mineBrowseHistoryCountLabel(preview.count) ?? '${preview.count} 条')
        : null;
  ...
  void _onTapProfile(BuildContext context, bool isLoggedIn) {
    if (!isLoggedIn) { Navigator.pushNamed(context, '/login'); return; }
    Navigator.pushNamed(context, '/settings');
  }

  void _onTapQuickAction(BuildContext context, String id, {AppLocalizations? l10n}) {
    if (id == 'search') { Navigator.pushNamed(context, '/search'); return; }
    if (id == 'reminders') { Navigator.pushNamed(context, '/reminders'); return; }
    if (id == 'settings') { Navigator.pushNamed(context, '/settings'); return; }
      ...
      builder: (dialogContext) {
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              const Text(AppReleaseInfo.appName, style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
              const SizedBox(height: 6),
              const Text('版本 ${AppReleaseInfo.fullVersion}', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
              const SizedBox(height: 12),
              Text(l10n?.mineAboutLegalese ?? '健康助手与药品信息辅助应用', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, height: 1.45)),
            ],
          ),
          actions: [
            TextButton(onPressed: () { Navigator.of(dialogContext).pop(); Navigator.pushNamed(context, '/user-agreement'); }, child: Text(dL10n?.legalUserAgreementTitle ?? '用户协议')),
            TextButton(onPressed: () { Navigator.of(dialogContext).pop(); Navigator.pushNamed(context, '/privacy-policy'); }, child: Text(dL10n?.legalPrivacyPolicyTitle ?? '隐私政策')),
            FilledButton(onPressed: () => Navigator.of(dialogContext).pop(), child: const Text('关闭')),
          ],
        );
      },
    );
