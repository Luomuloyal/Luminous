...
class _LoginPageState extends ConsumerState<LoginPage> {
  ...
  String _loginModeLabel(AuthLoginMode mode) =>
      mode == AuthLoginMode.password ? _l10n.authPasswordLoginMode : _l10n.authCodeLoginMode;

  String? _identifierValidator(String? value) =>
      ref.read(loginNotifierProvider.notifier).validateIdentifier(value ?? '', _identifierType);
  ...
  void _onLoginModeChanged(AuthLoginMode mode) {
    ...
    setState(() { _loginMode = mode; _codeCtrl.clear(); });
  ...
  Future<void> _onSendCode() async {
    ...
    final ok = await ref.read(loginNotifierProvider.notifier).sendCode(
  ...
  Future<void> _onLoginPressed() async {
    ...
    try {
      final result = await notifier.login(
        ...
        password: _passwordCtrl.text, code: _codeCtrl.text.trim(), mode: _loginMode,
      );
      ref.read(authServiceProvider).loginSuccess(result.user);
      ...
      if (!mounted) return;
      ToastUtils.instance.show(context, e.message.isNotEmpty ? e.message : '登录失败');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return AuthPageScaffold(children: [
      ...
      AuthHeroCard(palette: SoftBannerPalettes.authOf(context), icon: Icons.health_and_safety_rounded, title: l10n.loginHeroTitle, subtitle: l10n.loginHeroSubtitle(_identifierLabel(), _loginModeLabel(_loginMode))),
      const SizedBox(height: 10),
      AuthMethodSwitcher(items: [
        AuthMethodItem(label: _loginModeLabel(AuthLoginMode.password), selected: _loginMode == AuthLoginMode.password, onTap: () => _onLoginModeChanged(AuthLoginMode.password)),
        AuthMethodItem(label: _loginModeLabel(AuthLoginMode.code), selected: _loginMode == AuthLoginMode.code, onTap: () => _onLoginModeChanged(AuthLoginMode.code)),
      ]),
      ...
      _buildHelperText(),
      const SizedBox(height: 8),
      AuthLegalHint(onTapAgreement: () => Navigator.pushNamed(context, '/user-agreement'), onTapPrivacy: () => Navigator.pushNamed(context, '/privacy-policy')),
    ]);
  }
  ...
  Widget _buildTopBar() {
    ...
    return Row(children: [
      if (canPop) InkWell(
        ...
        child: Container(width: 32, height: 32, decoration: BoxDecoration(color: theme.cardTheme.color ?? scheme.surface, borderRadius: BorderRadius.circular(999), border: Border.all(color: scheme.outline)), child: Icon(Icons.arrow_back_ios_new_rounded, size: 16, color: scheme.onSurface)),
      ),
      const Spacer(),
      TextButton(
        onPressed: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => RegisterPage(authApi: _authApi, initialIdentifierType: AuthIdentifierType.email))),
        style: TextButton.styleFrom(minimumSize: const Size(52, 32), foregroundColor: scheme.primary, textStyle: const TextStyle(fontSize: 13.5, fontWeight: FontWeight.w700)),
  ...
  Widget _buildForm(LoginFormState form) {
    return Form(
      ...
      child: Column(children: [
        _buildTextField(controller: _identifierCtrl, label: _identifierLabel(), prefixIcon: _identifierType == AuthIdentifierType.phone ? Icons.phone_android_rounded : Icons.email_outlined, validator: _identifierValidator, onChanged: (v) => ref.read(loginNotifierProvider.notifier).onIdentifierChanged(v)),
        if (_loginMode == AuthLoginMode.password) ...[
          const SizedBox(height: 14),
          _buildTextField(controller: _passwordCtrl, label: _l10n.authPasswordLabel, prefixIcon: Icons.lock_outline_rounded, obscureText: _obscurePassword, validator: _passwordValidator, suffixIcon: IconButton(onPressed: () => setState(() => _obscurePassword = !_obscurePassword), icon: Icon(_obscurePassword ? Icons.visibility_off_rounded : Icons.visibility_rounded))),
          const SizedBox(height: 6),
          Align(alignment: Alignment.centerRight, child: TextButton(onPressed: () => ToastUtils.instance.show(context, _l10n.loginForgotPasswordPending), child: Text(_l10n.loginForgotPasswordAction))),
        ],
        if (_loginMode == AuthLoginMode.code) ...[
          const SizedBox(height: 14),
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Expanded(child: _buildTextField(controller: _codeCtrl, label: _l10n.authCodeLabel, prefixIcon: Icons.pin_rounded, validator: _codeValidator, keyboardType: TextInputType.number)),
  ...
  Widget _buildLoginButton(LoginFormState form) {
    ...
    return SizedBox(width: double.infinity, child: FilledButton(
      ...
      child: form.submitting ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : Text(_l10n.loginButton, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700)),
    ));
  }

  Widget _buildHelperText() {
    return Row(mainAxisAlignment: MainAxisAlignment.center, children: [
      Text('还没有账号？', style: TextStyle(fontSize: 13, color: Theme.of(context).colorScheme.onSurfaceVariant)),
      TextButton(onPressed: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => RegisterPage(authApi: _authApi, initialIdentifierType: AuthIdentifierType.email))), child: Text(_l10n.loginRegisterAction)),
    ]);
  }
}
