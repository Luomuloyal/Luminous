...
class RegisterNotifier extends Notifier<RegisterFormState> {
  ...
  static final _usernameRegExp = RegExp(r'^\S{2,30}$');
  ...
  void initCodeTarget(String identifier, String initialCode) {
    if (initialCode.trim().isNotEmpty) {
      _codeTarget = identifier.trim();
    }
  }
  ...
  String? validateUsername(String value) {
    ...
    if (!_usernameRegExp.hasMatch(username)) return '昵称需为2-30个非空字符';
    return null;
  }
  ...
  Future<RegisterResult> register({
    ...
    try {
      final response = type == AuthIdentifierType.email
